        ...
        stage('Build and Deploy FastAPI Backend') {
            steps {
               ...
               // sh "cd Backend && cd myvenv && . bin/activate"
                ...
                sh "sudo cd /home/dcnc/salesaccount.web/Backend && sudo pm2 start 'gunicorn main:app --workers 4 --worker-class uvicorn.workers.UvicornWorker --bind 0.0.0.0:55502 --timeout=3000' --name reconcileaibackend"
            }
        }
    }
}
